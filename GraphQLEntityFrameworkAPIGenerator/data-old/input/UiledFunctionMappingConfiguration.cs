using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;
using Microsoft.EntityFrameworkCore;

namespace WP.Cooking.GESE.WebAPI.Models;

[Table("UILedFunctionMappingConfigurations")]
public partial class UiledFunctionMappingConfiguration
{
    [Key]
    [Column("UILedFunctionMappingConfigId")]
    public Guid UiledFunctionMappingConfigId { get; set; }

    public string Description { get; set; } = null!;

    public byte Status { get; set; }

    [StringLength(100)]
    public string Owner { get; set; } = null!;

    [Column(TypeName = "datetime")]
    public DateTime Timestamp { get; set; }

    public Guid RevisionGroup { get; set; }

    public int Revision { get; set; }

    public byte TableTarget { get; set; }

    [Unicode(false)]
    public string? Notes { get; set; }

    [InverseProperty("UiledFunctionMappingConfig")]
    public virtual ICollection<UiledConfiguration> UiledConfigurations { get; set; } = new List<UiledConfiguration>();

    [InverseProperty("UiledFunctionMappingConfig")]
    public virtual ICollection<UiledFunctionMappingConfigurationsUiledFunctionMappingDetail> UiledFunctionMappingConfigurationsUiledFunctionMappingDetails { get; set; } = new List<UiledFunctionMappingConfigurationsUiledFunctionMappingDetail>();
}
