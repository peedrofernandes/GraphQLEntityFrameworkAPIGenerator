using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.ComponentModel.DataAnnotations.Schema;
using Microsoft.EntityFrameworkCore;

namespace WP.Cooking.GESE.WebAPI.Models;

[Table("UILedConfigurations")]
public partial class UiledConfiguration
{
    [Key]
    [Column("UILedConfigurationsId")]
    public Guid UiledConfigurationsId { get; set; }

    public string Description { get; set; } = null!;

    public byte Status { get; set; }

    [StringLength(100)]
    public string Owner { get; set; } = null!;

    [Column(TypeName = "datetime")]
    public DateTime Timestamp { get; set; }

    public Guid RevisionGroup { get; set; }

    public int Revision { get; set; }

    public byte TableTarget { get; set; }

    [Unicode(false)]
    public string? Notes { get; set; }

    [Column("UILedFunctionMappingConfigId")]
    public Guid? UiledFunctionMappingConfigId { get; set; }

    [InverseProperty("UiledConfigurations")]
    public virtual ICollection<UiledConfigurationsUiledDriverParameter> UiledConfigurationsUiledDriverParameters { get; set; } = new List<UiledConfigurationsUiledDriverParameter>();

    [ForeignKey("UiledFunctionMappingConfigId")]
    [InverseProperty("UiledConfigurations")]
    public virtual UiledFunctionMappingConfiguration? UiledFunctionMappingConfig { get; set; }

    [InverseProperty("UiledConfigurations")]
    public virtual ICollection<UiledGroupsConfiguration> UiledGroupsConfigurations { get; set; } = new List<UiledGroupsConfiguration>();
}
